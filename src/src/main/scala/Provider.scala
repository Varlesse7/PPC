...
object Provider {
    case class GetMeasure (res: Int)
    case class SendMeasure(meas: DataBaseActor.Measure)
    case class InitCond(conductor: ActorRef)
}



class Provider (db: ActorRef) extends Actor {
    import Provider._
    import Conductor._
  ...
  (partie1 zip partie2).map { case (l1, l2) => l1 ++ l2 }
    ...
    def receive = {
        ...
        case GetMeasure(res) => {
            db ! DataBaseActor.GetMeasure(partie(res)(cmpt%16))
            cmpt = cmpt + 1
        }
        case meas: DataBaseActor.Measure => {
            conductor ! SendMeasure(meas)
        }
    }
}